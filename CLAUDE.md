...
Sources are **javax.servlet-only** (`CodeEditorServlet.java`, `WEB-INF/web.xml`) — do not hand-write a jakarta variant. `org.eclipse.transformer:transformer-maven-plugin` rewrites the compiled bytecode and `web.xml` (via `src/main/transformer/*.properties`) to `jakarta.servlet`/Servlet 6.1 at build time, producing a classified sibling jar. A `maven-antrun-plugin` step then renames the two `target/` outputs so the **jakarta build becomes the default (no-suffix) artifact** and the **javax build becomes `-pre2606`** — this is intentionally inverted from a naive "javax is the default" scheme, because Polarion 2606 is this project's primary target going forward. `${project.build.finalName}` is set at the top-level `<build>` (not inside `maven-jar-plugin`'s own config) specifically so this rename step resolves the real on-disk jar name. Full pattern and pitfalls: see the `polarion-2606-jakarta-migration` skill.
...
- **`META-INF/hivemodule.xml`** — registers the admin page extender (`administrationPageExtenders`) that surfaces the Code Editor entry. A previous custom `customNavigationExtenders` sidebar contribution (`CodeEditorNavigationExtender`) was removed: it extended `com.polarion.alm.ui.server.navigation.NavigationExtender`, which Polarion 2606 removed entirely, and HiveMind aborts the *whole module* with FATAL when it can't resolve an interface — which also blocked this bundle's webapp mount. Do not reintroduce it. `plugin.xml` registers the webapp context root `polarion/code-editor`. Class names referenced from `hivemodule.xml` must match real Java classes or Polarion silently fails to load the contribution.
