...
import { openEditor, clickFile, doubleClickFile, waitForTab, reloadEditor, clearEditorStorage, tryCreateFile } from '../helpers/editor';
...
test.describe('Code Editor – Session & Cache Persistence', () => {
  ...
  test('all open tabs are restored after page reload', async ({ page }) => {
    ...
    // Double-click opens persistent tabs that are saved to session storage
    await doubleClickFile(page, SESSION_FILE_A);
    await waitForTab(page, SESSION_FILE_A);
    await doubleClickFile(page, SESSION_FILE_B);
  ...
  test('sidebar width persists across reload', async ({ page }) => {
    ...
    await page.mouse.move(resizerBox.x + 150, resizerBox.y + 10, { steps: 15 });
    await page.mouse.up();

    const widthBefore = await page.locator('#sidebar').evaluate((el: HTMLElement) => el.offsetWidth);
    // Only assert the drag was effective (not an absolute px threshold)
