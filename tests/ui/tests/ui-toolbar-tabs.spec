...
import { openEditor, clickFile, doubleClickFile, waitForTab, reloadEditor, clearEditorStorage, tryCreateFile, hasTab } from '../helpers/editor';
...
test.describe('Code Editor – Tab Management', () => {
  ...
  test('opening a second file adds a second tab', async ({ page }) => {
    // Double-click opens a persistent tab so both files remain open simultaneously
    await doubleClickFile(page, FILE_A);
    await waitForTab(page, FILE_A);
    await doubleClickFile(page, FILE_B);
  ...
  test('switching between tabs updates the editor content', async ({ page }) => {
    // Double-click to open both files as persistent tabs
    await doubleClickFile(page, FILE_A);
    await waitForTab(page, FILE_A);
    ...
    // Switch back to FILE_A via the tab bar
